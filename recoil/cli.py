...
def sentinel(
    ...
    focus: Optional[str] = typer.Option(
        None, "--focus", help="Steer the analysis, e.g. 'the Solana ecosystem' or 'stablecoin TVL'"
    ),
    ...
    if watch is not None:
        ...
        while True:
            code = _sentinel_once(out=out, skip_gate=skip_gate, focus=focus, tamper=tamper)
            print(f"[loop] run finished with exit {code}; next run in {interval}s")
            _time.sleep(interval)
    raise typer.Exit(code=_sentinel_once(out=out, skip_gate=skip_gate, focus=focus, tamper=tamper))
...
def _sentinel_once(
    *, out: Optional[str], skip_gate: bool, focus: Optional[str] = None, tamper: bool = False
    ...
    try:
        ...
        print(
            f" [2/4] report generated by {llm_stats['model']}:{focus_note} "
        ...
        if tamper:
            from .sentinel.agent import tamper_report

            report, fault = tamper_report(report, snapshot)
            typer.secho(
                f"  !!   FAULT INJECTED (demo): {fault}", fg=typer.colors.YELLOW, bold=True
            )
