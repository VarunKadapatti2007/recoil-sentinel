"""The three demo agent versions (plus historical parents) and their
deterministic behavior profiles.

The behavior profiles are the offline/mock execution path: given a triage
input they produce exactly the output the corresponding system prompt would
elicit from the model. This is what makes seeding and the demo path fully
deterministic and network-free. When an ANTHROPIC_API_KEY is present and
demo mode is off, the agent calls the live model instead (see triage.py).
"""
...
def _correct_triage(inp: dict[str, Any]) -> TriageOutput:
    """The policy-perfect triage decision — what v_good and v_fixed produce."""
...
def _v_regressed(inp: dict[str, Any]) -> TriageOutput:
    """The plausible 'improvement' edit: concise + only escalate during
    business hours. Silently breaks the after-hours-outage class."""
...
def _v_fixed(inp: dict[str, Any]) -> TriageOutput:
    """Keeps the concision improvement but preserves after-hours escalation."""
    out = _correct_triage(inp)
    words = out.reason.split()
    if len(words) > 20:
        out = out.model_copy(update={"reason": " ".join(words[:20])})
...
def _v1(inp: dict[str, Any]) -> TriageOutput:
    """Early version: several policy gaps (these failures seeded the suite)."""
    out = _correct_triage(inp)
    kind = inp.get("kind")
    if kind == "pii_leak":
...
def _v2(inp: dict[str, Any]) -> TriageOutput:
    """Second version: fixed v1's routing gaps, introduced new ones."""
    out = _correct_triage(inp)
    kind = inp.get("kind")
    if kind == "outage" and _is_after_hours(inp):
        # the original after-hours bug — the hero case was frozen from this failure
